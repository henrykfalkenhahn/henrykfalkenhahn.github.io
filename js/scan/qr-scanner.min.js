class e {
    ...
    constructor(a, b, c = this._onDecodeError, d = this._calculateScanRegion, f = "environment") {
        ...
        "number" === typeof d ? (this._legacyCanvasSize = d, console.warn("You're using a deprecated version of the QrScanner constructor which will be removed in the future")) : this._calculateScanRegion = d;
    ...
    hasFlash() {
        ...
        return a ? (new ImageCapture(a)).getPhotoCapabilities().then(a =>
            a.fillLightMode.includes("flash")).catch(a => {
            console.warn(a);
            return !1
    ...
    pause() {
        ...
        this._active && (this.$video.pause(), this._offTimeout || (this._offTimeout = setTimeout(() => {
            ...
            this._offTimeout = this.$video.srcObject =
                null
        }, 300)))
    }
        ...
        let h = c instanceof Worker,
            g = Promise.all([c || e.createQrEngine(), e._loadImage(a)]).then(([a, g]) => {
                ...
                return c instanceof Worker ? (h || c.postMessage({
                    ...
                    let f, l, g;
                    l = d => {
                        "qrResult" === d.data.type && (c.removeEventListener("message", l), c.removeEventListener("error", g), clearTimeout(f), null !== d.data.data ? a(d.data.data) : b(e.NO_QR_CODE_FOUND))
                    };
                    g = a => {
    ...
    static createQrEngine(a = e.WORKER_PATH) {
        return ("BarcodeDetector" in window ? BarcodeDetector.getSupportedFormats() : Promise.resolve([])).then(b => -1 !==
            b.indexOf("qr_code") ? new BarcodeDetector({
                formats: ["qr_code"]
            }) : new Worker(a))
    }
    _onPlay() {
    ...
    _scanFrame() {
        ...
        requestAnimationFrame(() => {
            1 >= this.$video.readyState ? this._scanFrame() : this._qrEnginePromise.then(a => e.scanImage(this.$video, this._scanRegion, a, this.$canvas)).then(this._onDecode, a => {
                this._active && (-1 !== (a.message || a).indexOf("service unavailable") && (this._qrEnginePromise = e.createQrEngine()), this._onDecodeError(a))
    ...
    static _drawToCanvas(a, b = null, c = null, d = !1) {
        ...
        d || (c.width = b && b.downScaledWidth ? b.downScaledWidth : h, c.height = b && b.downScaledHeight ? b.downScaledHeight : g);
        b = c.getContext("2d", {
            alpha: !1
        });
    ...
    static _loadImage(a) {
        ...
        if (a instanceof Image) return e._awaitImageLoad(a).then(() =>
            a);
